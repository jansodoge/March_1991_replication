...
#run the loop twice, respectively change the turnover parameter
data_averaged <- c()
for(run in seq(1, 80)){
print(run)
...
time = 1000  # iterations until equilibrium, a lazy solution to the steady state problem
m <- 30  # number of dimensions
n <- 50  # number of people


p3_list <- c(0.1) # turnover
...
averaged_data_vec_b <- rowMeans(data_averaged)
...
####
simulated_data <- data.frame(time = seq(1,1000), 
                             wo_turnover = average_data_vec_a,
                             w_turnover = tmp) %>% 
